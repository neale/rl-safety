...
class PPO(object):
    ...
    def save_checkpoint(self):
        ...
        if self.is_rand_agent:
            self.save_path = self.save_path + '_aup'
    ...
    def run_agents(self, steps_per_env):
        ...
        for _ in range(steps_per_env):
            if op.rnn_states_in is not None:
                ...
                #policies_aup = session.run(self.aup_agent.op_aup.policy, feed_dict={
                #    self.aup_agent.op_aup.states: [obs[-num_env:]]
                #})
            #for i, (env, policy, policy_aup, rnn_state) in enumerate(zip(self.envs, policies[0], policies_aup[0], new_rnn_states)):
            for i, (env, policy, rnn_state) in enumerate(zip(self.envs, policies[0], new_rnn_states)):
                action = np.random.choice(len(policy), p=policy)
                # action_aup = np.random.choice(len(policy), p=policy)
    ...
    def gen_training_batch(self, steps_per_env):
        ...
        # each environment.
        fd = {op.states: states}
        fd_aup = {op_aup.states: states}
        if op.rnn_states_in is not None:
            fd[op.rnn_states_in] = rnn_states
        policies, values = session.run([op.policy, op.v], feed_dict=fd)
        policies_aup, values_aup = session.run([op_aup.policy,
                                                op_aup.v],
                                                feed_dict=fd_aup)
        noop_qvalue = policies_aup[:, :, 0]  # NOOP is action 0
        noop_qvalue = np.expand_dims(noop_qvalue, axis=-1) # ok fine
        # now get maximal action
        max_qvalue = policies_aup.max(axis=2)
        max_qvalue = np.expand_dims(max_qvalue, axis=-1)

        penalty = np.absolute(max_qvalue - noop_qvalue)
        ...
        penalty = penalty[:-1, :, 0]
        scale = scale[:-1, :, 0]
        rewards = rewards - lamb*(penalty/scale) # AUP
        print(penalty.mean(), scale.mean(), (lamb*(penalty/scale)).mean())
    ...
    def train(self, aup_dict, model, total_steps=None):
        last_report = last_save = last_test = self.num_steps - 1
        total_steps = total_steps or self.total_steps
        self.state_encoder = aup_dict['vae']
        self.random_reward_fn = aup_dict['reward_fn']
        self.aup_agent = model
        self.is_rand_agent = False
