...
class SafeLifeBasePPO(ppo_aup.PPO):
    """
    ...
    def __init__(self, logdir=ppo_aup.DEFAULT_LOGDIR, **kwargs):
        rand_agent = True
        self.logdir = logdir
        ...
        super().__init__(envs, rand_agent, logdir=logdir, **kwargs)
            ...
            for step_num in range(self.benchmark_initial_steps):
                if random_policy:
                    ...
                    policies = self.session.run(
                        op_aup.policy, feed_dict={op_aup.states: [obs]})[0]
                obs = []
...
class SafeLifeAUP_example(SafeLifeBasePPO):
    """
    ...
    def build_logits_and_values(self, img_in, rnn_mask, use_lstm=False):
        ...
        if use_lstm:
            ...
            self.op_aup.rnn_states_in = lstm.zero_state(batch_shape[1], tf.float32)
            ...
            n, states, y = tf.while_loop(loop_cond, loop_body, (
                ...
                self.op_aup.rnn_states_in,
                tf.TensorArray(tf.float32, n_steps),
            ))
            self.op_aup.rnn_states_out = states
            self.op_aup.layer4 = y = y.stack()
        else:
            self.op_aup.layer4 = y = tf.layers.dense(
                y, units=512,
                activation=tf.nn.relu, kernel_initializer=ortho_init(np.sqrt(2)),
            )
        ...
        # Ad-hoc Q function
        """
        actions = tf.onehot(a_in, logits.shape[-1], dtype=tf.float32)
        self.op_aup.qlayer0 = qy
        self.op_aup.qlayer1 = qy = tf.layers.conv2d(
            qy, filters=32, kernel_size=5, strides=1,
            activation=tf.nn.relu, kernel_initializer=ortho_init(np.sqrt(2)),
        )
        self.op_aup.qlayer2 = qy = tf.layers.conv2d(
            qy, filters=64, kernel_size=3, strides=1,
            activation=tf.nn.relu, kernel_initializer=ortho_init(np.sqrt(2)),
        )
        self.op_aup.qlayer3 = qy = tf.layers.conv2d(
            qy, filters=64, kernel_size=3, strides=2,
            activation=tf.nn.relu, kernel_initializer=ortho_init(np.sqrt(2)),
        )
        qy_size = qy.shape[1] * qy.shape[2] * qy.shape[3]
        qy = tf.reshape(y, tf.concat([batch_shape, [qy_size]], axis=0))
        self.op_aup.qlayer4 = qy = tf.layers.dense(
            qy, units=512,
            activation=tf.nn.relu, kernel_initializer=ortho_init(np.sqrt(2)),
        )
        qvalues = tf.layers.dense(qy, units=len(self.gamma), kernel_initializer=ortho_init(1.0))
        """
